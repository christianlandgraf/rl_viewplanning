...
if __name__ == '__main__':
    ...
    viewpoint_filename = os.path.join(pkg_path, "config", "ipa_kifz_viewpoints.csv") 
   ...
   # Start viewpoint generation
    for i in range(1):
        ...
        for i in range(2000):
            ...
            print(observation[:7])
            if click.confirm('Do you want to save this viewpoint?', default=True):
                print("Save viewpoint")
                with open(viewpoint_filename, 'a') as f:
                    writer = csv.writer(f)
                    writer.writerow([observation[0], observation[1], observation[2],
